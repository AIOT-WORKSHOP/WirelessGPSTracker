...
typedef struct
{
    ...
    u8 timing;    //是否正在请求校时
    bool timed;   //是否已校时, 无线设备在未校时的时候一定要校一次时

    // epo/agps 文件信息
    u32 data_len;    // 消息包总大小
    u32 data_start;   // 请求文件时,数据起始位置
}SocketTypeAgpsExtend;
static SocketTypeAgpsExtend s_agps_socket_extend = {0,0,0,0,0};
...
static void agps_service_init_proc(void)
{
    ...
    if(gps_is_fixed())
    {
        agps_service_write_to_gps(); //here will read file
...
static void agps_service_data_finish_proc(void)
{
    ...
    else
    {
        ...
        {
            // 2019/7/8 0:0:0 即 1562515200  , 大于这个时间, 肯定校过时了
            if(last_gps_data.gps_time > 1562515200)
            {
                s_agps_socket_extend.timed = true;
                return;
            }
        }
    }
