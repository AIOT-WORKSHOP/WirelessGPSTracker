...
typedef struct
{
	...
	U32 g_sensor_eint_counts;

    U32 expmu_comm_err;
    U32 expmu_comm_cannot_establish;
    U32 expmu_pwr_on_cnt;
    U32 expmu_rtc_wkp_cnt;
    U32 expmu_exti1_wkp_cnt;
    U32 expmu_exti2_wkp_cnt;
    U32 expmu_time_value;
    U32 expmu_exti1_change;
    U32 expmu_exti2_change;
    U32 expmu_rtc_time_not_match;
...
#endif
static void auto_test_check_expmu(void);
static void auto_test_wifi(void);
...
bool auto_test_is_working(void)
{
    u8 imei[17] = {0};
    
    if(config_service_is_test_mode() == false)
    {
        gsm_get_imei(imei);
        
        if(GM_memcmp(imei,"668613000000000",15) != 0)
        {
            return false;
        }
    }
...
{
	//IMEI
	gsm_get_model_imei((U8*)s_auto_test.test_result.imei);
    ...
    //MCU版本
    expmu_get_version_string(s_auto_test.test_result.expmu_version);
	auto_test_check_chip_rid();
    #if GW_DEV_CODE_SUPPORT
	...
	#endif
    auto_test_check_expmu();
   	if (s_auto_test.is_self_check)
	{
		
		s_auto_test.test_result.required_items_test_pass = ((s_auto_test.test_result.location_data_counts > s_auto_test.test_result.test_time_seconds/CONFIG_UPLOAD_TIME_DEFAULT*0.8) &&
	...
	else
	{
		...
		s_auto_test.test_result.required_items_test_pass = ((s_auto_test.test_result.heart_counts >= 1) &&
														    ...
														    (s_auto_test.test_result.g_sensor_eint_counts > 2));
        #else
        s_auto_test.test_result.required_items_test_pass = ((s_auto_test.test_result.location_data_counts >= 1) &&
														    (s_auto_test.test_result.test_time_seconds > 30));
        #endif
	}
	...
	if(s_auto_test.test_result.test_time_seconds % SECONDS_PER_MIN == 0)
	{
        ...
        /*mcu的信息也要一分钟保存一次*/
        //smsp_set_save_fram_command();
	}
	if(!s_auto_test.is_self_check)
	{
		char test_result[2048] = {0};
		auto_test_get_result(false,test_result);	
		GM_snprintf(test_result + GM_strlen(test_result), (2048 - GM_strlen(test_result)), "\r\n");
...
static void struct_to_jsonstr(const AutoTestResult result,char* json_str)
{
    ...
    json_add_string(p_result_root, "mcu_version",result.expmu_version);
	...
	json_add_int(p_result_root, "reboot_counts_fail", result.reboot_counts_fail);
	...
	//json_add_int(p_result_root, "g_sensor_eint_counts", result.g_sensor_eint_counts);
    json_add_int(p_result_root, "mcu_power_on_counts",result.expmu_pwr_on_cnt);
    json_add_int(p_result_root, "mcu_rtc_wkp_counts",result.expmu_rtc_wkp_cnt);
    json_add_int(p_result_root, "mcu_light_sensor_wkp_counts",result.expmu_exti1_wkp_cnt);
    json_add_int(p_result_root, "mcu_exit2_wkp_counts",result.expmu_exti2_wkp_cnt);
    json_add_int(p_result_root, "mcu_rtc_time",result.expmu_time_value);
    if(result.expmu_time_value > 1564471022)
    {
        json_add_string(p_result_root,"calibration_mcu_time","Done");
    }
    else
    {
        json_add_string(p_result_root,"calibration_mcu_time","None");
    }
    json_add_int(p_result_root, "mcu_rtc_time_not_match_system_time",result.expmu_rtc_time_not_match);
    json_add_int(p_result_root, "mcu_light_sensor_change_counts",result.expmu_exti1_change);
    json_add_int(p_result_root, "reming_gps_data_counts",i);
    json_add_int(p_result_root, "mcu_comm_error_counts",result.expmu_comm_err);
    json_add_int(p_result_root, "mcu_comm_cannot_establish_counts",result.expmu_comm_cannot_establish);
    json_add_int(p_result_root, "send_location_data_success_counts",result.location_data_counts);
    json_add_int(p_result_root, "first_ap_rssi",abs(result.first_ap_rssi));
    json_add_int(p_result_root, "second_ap_rssi",abs(result.second_ap_rssi));
...
void auto_test_set_gps_location_counts(void)
{
    s_auto_test.test_result.location_data_counts++;
}
...
static void auto_test_check_expmu(void)
{ 
    static u8 pre_state = 0xFF;
    
    s_auto_test.test_result.expmu_pwr_on_cnt = expmu_get_powr_on_cnt();

    s_auto_test.test_result.expmu_exti1_wkp_cnt = expmu_get_exti1_wkp_cnt();

    s_auto_test.test_result.expmu_rtc_wkp_cnt = expmu_get_rtc_wkp_cnt();

    s_auto_test.test_result.expmu_comm_err = system_state_get_expmu_comm_err_count();

    s_auto_test.test_result.expmu_exti2_wkp_cnt = expmu_get_exti2_wkp_cnt();

    s_auto_test.test_result.expmu_time_value = expmu_get_rtc_time_value();

    s_auto_test.test_result.expmu_rtc_time_not_match = system_state_get_rtc_time_not_match_count();

    s_auto_test.test_result.expmu_comm_cannot_establish = system_state_get_expmu_smsp_comm_cannot_establish();

    auto_test_period_read_expmu_reg(EXPMU_GPIOC_REG);

    if(pre_state != auto_test_get_expmu_light_io_state())
    {
        if(pre_state != 0xFF)
        {
            s_auto_test.test_result.expmu_exti1_change++;
        }

        pre_state = auto_test_get_expmu_light_io_state();
    }
}

u8 get_max_ap_rssi(u8 invalid)
{
	u8 rssi = 0;
	
	u8 last_index = 0xFF ,index= 0;

    wifi_scan_result_struct *wifi;
    
    wifi = wifi_service_get_wifi_scan_result();

	for(index = 0; index < wifi->info.scan_ap_num; index++)
	{
		if((u8)wifi->info.scan_ap[index].rssi > rssi)
		{
			if(invalid != index)
			{
				rssi = wifi->info.scan_ap[index].rssi;
...
static void auto_test_wifi(void)
{
    ...
    if(wifi == NULL)
    {
        s_auto_test.test_result.first_ap_rssi = -100;

        s_auto_test.test_result.second_ap_rssi = -100;
    }
    else
    {
        index = get_max_ap_rssi(0xFF);
        
        s_auto_test.test_result.first_ap_rssi = (index == 0xFF)?(-100):(wifi->info.scan_ap[index].rssi);

        index = get_max_ap_rssi(index);

        s_auto_test.test_result.second_ap_rssi = (index == 0xFF)?(-100):(wifi->info.scan_ap[index].rssi);
    }
}
...
static void auto_test_check_gsensor(void)
{
	...
	s_auto_test.test_result.g_sensor_eint_counts = g_sensor_get_shake_event_count_when_eint();
