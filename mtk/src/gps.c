...
GM_ERRCODE gps_power_on(const U16 report_lbs_time,const U16 report_interval)
{
	GM_ERRCODE ret = GM_SUCCESS;
	LOG(INFO,"report_lbs_time=%d,report_interval=%d",report_lbs_time,report_interval);
	...
	s_gps.report_lbs_time = report_lbs_time;
	s_gps.report_interval = report_interval;
	...
	switch (s_gps.gpsdev_type)
	{
		...
		case GM_GPS_TYPE_UNKNOWN:
		{
			...
			if(GM_SUCCESS != ret)
			{
				LOG(ERROR,"Failed to open GPS UART");
			}
	...
	if(s_gps.report_lbs_time == 0)
	{
		//立即上报一条LBS，再等1分钟GPS（有可能会上报两条LBS）
		gps_service_push_lbs();
		GM_StartTimer(GM_TIMER_GPS_CHECK_STATE, TIM_GEN_1SECOND*SECONDS_PER_MIN, check_fix_state);
	}
	else
	{
		GM_StartTimer(GM_TIMER_GPS_CHECK_STATE, TIM_GEN_1SECOND*s_gps.report_lbs_time, check_fix_state);
	}
...
static void check_fix_state(void)
{
	...
	{
		GM_StartTimer(GM_TIMER_GPS_CHECK_STATE, TIM_GEN_1SECOND*s_gps.report_interval, check_fix_state);
	}
}
...
GM_ERRCODE gps_write_agps_info(const float lng,const float lat,const U8 leap_sencond,const U32 data_start_time)
{
	...
	if (GM_SUCCESS)
	{
		LOG(ERROR,"Failed to write_agps_info,ret=%d",ret);
	}
	return ret;
}
...
time_t get_last_fixed_time(void)
{
    return s_gps.last_fixed_time;
}
...
static void check_fix_state_change(void)
{
	GM_CHANGE_ENUM change = util_check_state_change(s_gps.is_3d,&s_gps.state_record,10,20);
	...
	if (GM_CHANGE_TRUE == change)
	{
		...
		s_gps.last_fixed_time = util_clock();
...
static void upload_gps_data(const GPSData current_gps_data)
{
	...
	{
        GM_StopTimer(GM_TIMER_GPS_CHECK_STATE);
		LOG(INFO,"Upload GPS once");
		gps_service_push_one_gps(&current_gps_data);
		gps_power_off();
		
	}
	else
	{
		if ((util_clock() - s_gps.report_time) >= s_gps.report_interval)
		{
			LOG(INFO,"Upload GPS between %d seconds",s_gps.report_interval);
			gps_service_push_one_gps(&current_gps_data);
			s_gps.report_time = util_clock();
		}
	}
}
...
static void on_received_rmc(const NMEASentenceRMC rmc)
{
	...
	if (seconds_from_reboot - s_gps.save_time >= 1)
	{
		...
		if(!system_state_has_reported_gps_since_boot())
		{
			tm_time = util_gmtime(gps_data.gps_time);
