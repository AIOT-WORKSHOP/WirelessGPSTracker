...
static void expmu_port_receive(void* msg);
...
GM_ERRCODE uart_create(void)
{
    ...
    GM_RegisterCallBack(GM_CB_UART3_RECEIVE, (U32)expmu_port_receive);
	
	g_uart.inited = true;

	uart_open_port(GM_UART_DEBUG,BAUD_RATE_HIGH,0);

    uart_open_port(GM_UART_EXPMU,115200,0);
...
static void expmu_port_receive(void* msg)
{
	...
	if (false == g_uart.UARTParas[GM_UART_EXPMU].is_open)
	{
		return;
	}
	
	g_uart.UARTParas[GM_UART_EXPMU].last_rcv_time = util_clock();	

	do
	{
		GM_memset(g_uart.UARTParas[GM_UART_EXPMU].rcv_buff, 0, GM_UART_RCV_BUFF_LEN);
		len = GM_UartRead((Enum_SerialPort)GM_UART_EXPMU, (U8*)g_uart.UARTParas[GM_UART_EXPMU].rcv_buff, GM_UART_RCV_BUFF_LEN);
		if (len > 0)
		{     
			parse_expmu_dat_fun((U8*)g_uart.UARTParas[GM_UART_EXPMU].rcv_buff,len);
		}
		else
		{
			break;
