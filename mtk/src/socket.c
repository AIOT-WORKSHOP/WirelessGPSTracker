...
{  
    ...
    LOG(INFO,"clock(%d) send to %s len %d:",util_clock(),socket->addr,len);
    
    log_service_print_hex((const char*)data,len);
    ...
    {
        /*TCP头部长度*/
        sta->gprs_data_traffic_upstream += (40 + len);

    }
    else if(socket->type == STREAM_TYPE_DGRAM)
    {
        /*UDP头部长度*/
        sta->gprs_data_traffic_upstream += (28 + len);
    }
...
GM_ERRCODE gm_socket_recv(SocketType *socket)
{
    ...
    do
    {
        ...
        if (recvlen > 0)
        {
           // gm_socket_get_ackseq(socket, &socket->ack_seq);
            ...
            {
                /*TCP头部长度*/
                sta->gprs_data_traffic_downstream += (40 + recvlen);

            }
            else if(socket->type == STREAM_TYPE_DGRAM)
            {
                /*UDP头部长度*/
                sta->gprs_data_traffic_downstream += (28 + recvlen);
            }

            LOG(INFO,"clock(%d) recv from %s len %d:",util_clock(),socket->addr,recvlen);

            if((socket->access_id != SOCKET_INDEX_UPDATE)&&(socket->access_id != SOCKET_INDEX_UPDATE_FILE))    
            log_service_print_hex(recv_buff,recvlen);
